...
from jointsbm.utils.utils import *
...
class JointSBM(object):
    ...
    def prepare_data(self):
        ...
        for i in tqdm(range(self.n_graphs)):
            Qn = get_Qn(self.graphs[i], self.K) * np.sqrt(
                self.total_nodes * 1.0 / self.n_nodes_array[i]
            )
            Q.append(Qn)
        ...
        while stopValue > self.tol and iter < self.maxIter:
            ...
            if self.parallel:
                ...
                X = Parallel(n_jobs=num_cores)(
                    delayed(processLoop)(Q[n], W, X[n], V, deltas_[0][n], K)
            ...
            if np.sum(counts_memberships == 0.0) > 0:
                iter = 1
                logging.warning("Restarting...")
                X = [
                    np.random.multinomial(1, [1.0 / K] * K, size=n_nodes_array[i])
            ...
            for n in range(n_graphs):
                ...
                term = np.sqrt(np.matmul(inv(deltas_[1]), deltas_[0][n])) + np.sqrt(
                    np.eye(K) * 1.0 / rt
                )
                loss[n] = (
                    deltas_[2][n] * frobenius_norm(XW - Q[n]) ** 2
                    + frobenius_norm(np.matmul(Q[n], term)) ** 2
                )
            t1 = time.time()
    ...
    def evalutate(self, memberships):
        ...
        return {
            "NMI": {"nmi": np.mean(individual_nmi), "overall_nmi": overall_nmi},
            "ARI": {"ari": np.mean(individual_ari), "overall_ari": overall_ari},
            "MCR": {"mcr": np.mean(individual_mcr), "overall_mcr": overall_mcr},
        }
